...
const TitleMaster = ({ darkMode }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (editingItem) {
      setTitles((prev) => prev.map((item) => (item.id === editingItem.id ? { ...item, ...formData } : item)))
    } else {
      setTitles((prev) => [
        ...prev,
        {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Title Master</h1>
      ...
      {showForm && (
        <Card className="dark:bg-gray-800">
          <CardHeader>
            <CardTitle>{editingItem ? "Edit" : "Add"} Title</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                {filteredTitles.map((title) => (
                  <tr key={title.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Badge variant={title.status === "Active" ? "default" : "secondary"}>{title.status}</Badge>
                    </td>
