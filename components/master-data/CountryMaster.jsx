...
const CountryMaster = ({ darkMode }) => {
  ...
  const [formData, setFormData] = useState({ name: "", code: "", dialCode: "", status: "Active" })
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (editingItem) {
      setCountries((prev) => prev.map((item) => (item.id === editingItem.id ? { ...item, ...formData } : item)))
    } else {
      setCountries((prev) => [
        ...prev,
        {
    ...
    setFormData({ name: "", code: "", dialCode: "", status: "Active" })
    setShowForm(false)
    setEditingItem(null)
  }
  ...
  const handleEdit = (item) => {
    setEditingItem(item)
    setFormData({ name: item.name, code: item.code, dialCode: item.dialCode, status: item.status })
    setShowForm(true)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Country Master</h1>
      ...
      {showForm && (
        <Card className="dark:bg-gray-800">
          <CardHeader>
            <CardTitle>{editingItem ? "Edit" : "Add"} Country</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-2">Dial Code</label>
                  <Input
                ...
                {filteredCountries.map((country) => (
                  <tr key={country.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                      <Button size="sm" variant="ghost" onClick={() => handleEdit(country)}>
